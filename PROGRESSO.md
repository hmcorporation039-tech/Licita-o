...
### Bloco 3 — Caminho da IA (entregue desligado)

Corrigido em código e entregue atrás de `AI_ANALYSIS_ENABLED=false`. A verificação
ponta a ponta contra a API real está pendente e é uma decisão de gasto.

| # | Defeito | Correção | Status |
|---|---|---|---|
| P1-07 | Edital truncado em 200.000 caracteres e apenas 1 documento analisado — habilitação e Termo de Referência ficavam de fora | PDF nativo (resolve edital escaneado), conjunto documental de até 5 PDFs por prioridade, sem truncamento | 🟡 |
| P1-08 | Análise rodando dentro da requisição HTTP; `max_tokens` baixo demais | Fila `analise` dedicada, resposta `202`, streaming com `finalMessage()`, `max_tokens` 32k, polling no frontend | 🟡 |
